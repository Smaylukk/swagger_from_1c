...
Процедура ЗагрузитьПроект(Команда)
	ДВФ = ОбщегоНазначенияСайтКлиент.ПолучитьДиалогВыбораКаталога("Выберите каталог с проектом");
...
Функция СоздатьКаталоги(Путь)
	Возврат Справочники.Ресурсы.СоздатьКаталоги(Путь);
КонецФункции

&НаСервереБезКонтекста
Функция СоздатьРесурс(АдресФайла, Путь, ИмяФайла, Расширение)
	Возврат Справочники.Ресурсы.СоздатьРесурс(АдресФайла, Путь, ИмяФайла, Расширение);
КонецФункции
...
Процедура ЗагрузитьНаборКартинок(Команда)
	ДВФ = ОбщегоНазначенияСайтКлиент.ПолучитьДиалогВыбораФайла("Выберите картинки", ОбщегоНазначенияСайтКлиент.ПолучитьФильтр_ВсеКартинки(), Истина);
