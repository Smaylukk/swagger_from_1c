...
Функция ПолучитьТекстМакета(ВходящиеПараметры) Экспорт 
	...
	Для каждого ТекСтрокаСостава Из Состав Цикл
		Если ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.Запрос") Тогда
			Результат = Результат + ПолучитьТекстПоЗапросу(ТекСтрокаСостава, ВходящиеПараметры) + Символы.ПС;
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.ПростойТекст") Тогда
			Результат = Результат + ТекСтрокаСостава.Текст + Символы.ПС;
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.Значение") Тогда
			
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.Объект") Тогда
			Результат = Результат + Строка(ТекСтрокаСостава.ИсточникДанных) + Символы.ПС;
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.Список") Тогда
			
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.ЗапросСОбработкойРезультата") Тогда
			Результат = Результат + ПолучитьТекстПоЗапросуСОбработкой(ТекСтрокаСостава, ВходящиеПараметры) + Символы.ПС;
		ИначеЕсли ТекСтрокаСостава.ВидИсточникаДанных = ПредопределенноеЗначение("Перечисление.ВидыИсточниковДанных.ПроизвольныйКод") Тогда
...
Функция ПолучитьТекстПоЗапросу(СтрокаСостава, ВходящиеПараметры)
	...
	Если Ложь Тогда
		ИсточникОб = Справочники.ИсточникДанных.СоздатьЭлемент()
	КонецЕсли;
...
Функция ПолучитьТекстПоЗапросуСОбработкой(СтрокаСостава, ВходящиеПараметры)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			СобытияЖурналаРегистрации.Добавить(Новый Структура("ИмяСобытия, ПредставлениеУровня, Комментарий, ДатаСобытия", "МакетыБлоков.ОшибкаВыполнения", "Ошибка", текстОшибки, ТекущаяДата()));
		КонецПопытки;
...
Функция ПолучитьТекстПроизволныйКод(СтрокаСостава, ВходящиеПараметры)
	...
	Исключение
		СобытияЖурналаРегистрации = Новый СписокЗначений;
		СобытияЖурналаРегистрации.Добавить(Новый Структура("ИмяСобытия, ПредставлениеУровня, Комментарий, ДатаСобытия", "МакетыБлоков.ОшибкаВыполнения", "Ошибка", ОписаниеОшибки() + " - " + КодВыполнения, ТекущаяДата()));
		ЖурналРегистрации.ЗаписатьСобытияВЖурналРегистрации(СобытияЖурналаРегистрации);
	КонецПопытки;
	Результат = Результат + ВыходнаяСтрока + Символы.ПС; 
