...
Функция ПолучитьСтраницу(ВидСтраницы)
	Результат = Справочники.Страницы.ПустаяСсылка();
	...
	|ИЗ
	|	Справочник.Страницы КАК Страницы
	|ГДЕ
...
Функция Страница404(ДанныеПользователя) Экспорт 
	Страница = ПолучитьСтраницу(ПредопределенноеЗначение("Перечисление.ВидыСтраниц.Страница404"));
	Если Страница.Пустая() Тогда
...
Функция ПолучитьТекстСтраницы(Страница, ДанныеПользователя)
	Если Ложь Тогда
		Страница = Справочники.Страницы.СоздатьЭлемент()
	КонецЕсли;
...
Функция ПолучитьСтрокуСтилей(Страница)
	...
	Для Каждого ТекСтиль Из Страница.Стили Цикл
		...
		Иначе
			СетевойКаталог = "";
			ЛокальныйКаталог = "/" + ОбщегоНазначенияСайтПовтИсп.ИмяПубликации() + "/res/";
...
Процедура ОбработатьТекстСтраницы(Текст, ДанныеПользователя) Экспорт
	...
	Текст = СтрЗаменить(Текст, "{#Имя_Публикации#}", ОбщегоНазначенияСайтПовтИсп.ИмяПубликации());
...
Функция ПолучитьСсылкиИзображений()
	...
	|ИЗ
	|	Справочник.Ресурсы КАК Изображения
	|ГДЕ
...
Функция ПолучитьСсылкиФайлов()
	...
	|ИЗ
	|	Справочник.Ресурсы КАК ФайлыHTTP
	|ГДЕ
...
Процедура ВставитьСсылкиНаИзображения(Текст)
	...
	Пока Истина Цикл
		...
		Если ПозНач > 0 Тогда
			...
			Изображение = Справочники.Ресурсы.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД));
...
Процедура ВставитьСсылкиНаФайлы(Текст)
	...
	Пока Истина Цикл
		...
		Если ПозНач > 0 Тогда
			...
			Файл = Справочники.Ресурсы.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД));
...
Процедура ВставитьСсылкиНаРесурсы(Текст)
	...
	Пока Истина Цикл
		...
		Если ПозНач > 0 Тогда
			...
			Ресурс = Справочники.Ресурсы.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД));
...
Процедура ВставитьБлокиТекстов(Текст, ДанныеПользователя)
	Пока Истина Цикл
		...
		Если ПозНач > 0 Тогда
			...
			Макет = Справочники.МакетыБлоков.ПолучитьСсылку(Новый УникальныйИдентификатор(УИД));
			Если Не Макет.Пустая() Тогда
				вхПараметры = Новый Структура;
				//ФиксированныеПараметры
				вхПараметры.Вставить("Пользователь", ДанныеПользователя.Пользователь);
...
Процедура ОбработатьТекстСтраницыДляПроверки(Текст) Экспорт
	Текст = СтрЗаменить(Текст, "/{#Имя_Публикации#}", Константы.ПолныйАдресПубликации.Получить());	
КонецПроцедуры

// Функция ПолучитьТекстПеременной
//
// Описание:
// возвращает текстовое содержимое,
// которым надо заменить соответсвующий кусок кода
// Параметры (название, тип, дифференцированное значение)
//  ИмяПеременной, Строка
// Возвращаемое значение: 
//  Строка
Функция ПолучитьТекстПеременной(ИмяПеременной)
    Результат = Неопределено;
	
	
	
	Возврат Результат;
КонецФункции //ПолучитьТекстПеременной
...
Функция ПолучитьСписокПеременных() Экспорт
	...
	Результат.Добавить("{#Имя_Публикации#}", "Имя публикации");
	Результат.Добавить("{#ИмяПользователя#}", "Имя пользователя");
	Результат.Добавить("{#ОшибкаАвторизации#}", "Ошибка авторизации");
	
	//переменные 
	Результат.Добавить("{#Наименование#}", "Наименование");
	Результат.Добавить("{#ОбъектАвторизации#}", "Объект авторизации");
...
Функция ПолучитьСписокСсылок() Экспорт
	...
	Результат.Добавить("/{#Имя_Публикации#}/index", "Стартовая страница");
	Результат.Добавить("/{#Имя_Публикации#}/index/login", "Страница авторизации");
	Результат.Добавить("/{#Имя_Публикации#}/index/logout", "Страница завершения сеанса");
	//Результат.Добавить("/{#Имя_Публикации#}/index/order", "Страница заказа");
	//Результат.Добавить("/{#Имя_Публикации#}/index/orders", "Страница списка заказов");
	//Результат.Добавить("/{#Имя_Публикации#}/index/good", "Страница товара");
	//Результат.Добавить("/{#Имя_Публикации#}/index/goods", "Страница списка товаров");
	//
...
Функция ПолучитьРесурсВОтвет(ОтносительныйURL) Экспорт 
	...
	Иначе
		...
		Если Выборка.ВидРесурса = ПредопределенноеЗначение("Перечисление.ВидыРесурса.Изображение") Тогда
			Ответ.УстановитьТелоИзДвоичныхДанных(Выборка.Файл.Получить());
			
			Ответ.Заголовки.Вставить("accept-ranges", "bytes");
		ИначеЕсли Выборка.ВидРесурса = ПредопределенноеЗначение("Перечисление.ВидыРесурса.Стиль") Тогда
			...
			КонецЕсли;
		ИначеЕсли Выборка.ВидРесурса = ПредопределенноеЗначение("Перечисление.ВидыРесурса.Скрипт") Тогда
			...
			Иначе
				...
				Ответ.УстановитьТелоИзДвоичныхДанных(ДД_Файла);
			КонецЕсли;
		ИначеЕсли Выборка.ВидРесурса = ПредопределенноеЗначение("Перечисление.ВидыРесурса.Файл") Тогда
			Если Выборка.ТекстовыйФайл Тогда
				Текст = Выборка.Текст;
				Ответ.УстановитьТелоИзСтроки(Текст);
			Иначе
